...
class BerlinSpider(scrapy.Spider):
    ...
    custom_settings = {
        "USER_AGENT": "jedeschule (open data project)",
        "DOWNLOAD_DELAY": 1,
    }
    ...
    def parse_detail(self, response):
        meta = {}
        name = (
            response.css("#ContentPlaceHolderMenuListe_lblSchulname::text")
            .extract_first()
            .strip()
        )  # .rsplit('-', 1)
        meta["name"] = self.fix_data(name)
        meta["id"] = self._parse_school_no(response.url)
        meta["address"] = self.fix_data(
            response.css(
                "#ContentPlaceHolderMenuListe_lblStrasse::text"
            ).extract_first()
        )
        meta["zip"], meta["city"] = self.fix_data(
            response.css("#ContentPlaceHolderMenuListe_lblOrt::text").extract_first()
        ).split(" ", 1)
        schooltype = re.split(
            "[()]",
            response.css(
                "#ContentPlaceHolderMenuListe_lblSchulart::text"
            ).extract_first(),
        )
        meta["schooltype"] = self.fix_data(schooltype[0].strip())
        meta["legal_status"] = self.fix_data(schooltype[1].strip())
        meta["telephone"] = self.fix_data(
            response.css(
                "#ContentPlaceHolderMenuListe_lblTelefon::text"
            ).extract_first()
        )
        meta["fax"] = self.fix_data(
            response.css("#ContentPlaceHolderMenuListe_lblFax::text").extract_first()
        )
        meta["mail"] = self.fix_data(
            response.css(
                "#ContentPlaceHolderMenuListe_HLinkEMail::text"
            ).extract_first()
        )
        meta["web"] = self.fix_data(
            response.css(
                "#ContentPlaceHolderMenuListe_HLinkWeb::attr(href)"
            ).extract_first()
        )
        headmaster = response.css(
            "#ContentPlaceHolderMenuListe_lblLeitung::text"
        ).extract_first()
        if headmaster:
            meta["headmaster"] = self.fix_data(
                " ".join(headmaster.split(",")[::-1]).strip()
            )
        meta["cookiejar"] = response.meta["cookiejar"]
        meta["data_url"] = response.url
        activities = self.fix_data(
            response.css("#ContentPlaceHolderMenuListe_lblAGs::text").extract_first()
        )
        if activities:
            meta["activities"] = [x.strip() for x in activities.split(";")]
        partner = self.fix_data(
            response.css(
                "#ContentPlaceHolderMenuListe_lblPartner::text"
            ).extract_first()
        )
        if partner:
