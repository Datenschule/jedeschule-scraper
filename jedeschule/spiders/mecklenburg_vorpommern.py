...
class MecklenburgVorpommernSpider(SchoolSpider):
    ...
    def parse(self, response):
        ...
        relevant_title = next(
            title
            for title in titles
            if "Download" in title and "Schulverzeichnis" in title
        )
    ...
    def parse_xml(self, response: scrapy.http.Response):
        ...
        for sheet in relevant_sheets:
            ...
            for row_number in range(worksheet.nrows):
                ...
                try:
                    row_data["Schul-behörde"] = legend[
                        row_data["Schul-behörde"].replace("\n", "")
                    ]
                except:
                    row_data["Schul-behörde"] = "unknown"

                try:
                    row_data["Landkreis/ kreisfr. Stadt"] = legend[
                        row_data["Landkreis/ kreisfr. Stadt"].replace("\n", "")
                    ]
                except:
                ...
                try:
                    if not "BLS" in sheet:
                        row_data["Schulart/ Org.form"] = legend[
                            row_data["Schulart/ Org.form"].replace("\n", "")
                        ]
                    else:
