...
class SachsenSpider(SchoolSpider):
    ...
    def normalize(item: Item) -> School:
        ...
        school["fax"] = "".join(
            [building.get("fax_code") or "", building.get("fax_number") or ""]
        )
        school["phone"] = "".join(
            [building.get("phone_code_1") or "", building.get("phone_number_1") or ""]
        )
        school["latitude"] = building.get("latitude")
        school["longitude"] = building.get("longitude")
        school["school_type"] = helper.resolve_school_type(
            building.get("school_type_keys", [None])[0]
        )
